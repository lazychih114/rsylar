...
class DeferClass {
 public:
  ...
  DeferClass(DeferClass&&) noexcept = default;
  DeferClass& operator=(DeferClass&&) noexcept = default;
...
// helper to deduce template parameter for DeferClass when CTAD is not available
// helper that accepts a lambda via operator+ and returns a DeferClass with the
// correct deduced template parameter. This allows the pattern:
//   DEFER { /* code */ };
// to expand to:
//   auto defer_placeholderN = DeferHelper() + [&]() { /* code */ };
// which compiles on C++11 without CTAD.
struct DeferHelper {
  template <class F>
  DeferClass<F> operator+(F&& f) const {
    return DeferClass<F>(std::forward<F>(f));
  }
};

#define _CONCAT(a, b) a##b
// use DeferHelper() + [&]() { ... } pattern
#define _MAKE_DEFER_(line) auto _CONCAT(defer_placeholder, line) = DeferHelper() + [&]()
