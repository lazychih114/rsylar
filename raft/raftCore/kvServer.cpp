...
KvServer::KvServer(int me, int maxraftstate, std::string nodeInforFileName, short port) : m_skipList(6) {
  ...
  sylar::IOManager::GetThis()->schedule([this, port]() -> void {
    // provider是一个rpc网络服务对象。把UserService对象发布到rpc节点上
    auto provider = std::make_shared<sylar::rpc::RpcProvider>();
    m_provider = provider;
    provider->NotifyService(this);
    // NotifyService 接收原始指针，确保 Raft 对象生命周期为 shared_ptr 管理
  ...
  sylar::IOManager::GetThis()->schedule(std::bind(&KvServer::ReadRaftApplyCommandLoop, this));
}
