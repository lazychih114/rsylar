  ...
  if (m_status == Leader) {
    ...
    for (int i = 0; i < m_peers.size(); i++) {
      ...
      if (m_nextIndex[i] <= m_lastSnapshotIncludeIndex) {
        ...
        // t.detach();
        m_ioManager->schedule([this, i]() { this->leaderSendSnapShot(i); });
        continue;
      }
      //构造发送值
      int preLogIndex = -1;
...
bool Raft::sendRequestVote(int server, std::shared_ptr<raftRpcProctoc::RequestVoteArgs> args,
  ...
  // todo
  auto start = now();
  SYLAR_LOG_INFO(g_logger) << format("[func-sendRequestVote rf{%d}] 向server{%d} 发送 RequestVote 开始, args->lastLogIndex():{%d}",
                                              m_me, server, args->lastlogindex());
  bool ok = m_peers[server]->RequestVote(args.get(), reply.get());
  SYLAR_LOG_INFO(g_logger) << format("[func-sendRequestVote rf{%d}] 向server{%d} 发送 RequestVote 完畢, args->lastLogIndex():{%d}, 耗时:{%d} ms",
                                              m_me, server, args->lastlogindex(), now() - start);
...
void Raft::init(std::vector<std::shared_ptr<RaftRpcUtil>> peers, int me, std::shared_ptr<Persister> persister,
  ...
  m_ioManager->schedule([this]() -> void { this->leaderHearBeatTicker(); });
  m_ioManager->schedule([this]() -> void { this->electionTimeOutTicker(); });
  m_ioManager->schedule([this]() -> void { this->applierTicker(); });
}
