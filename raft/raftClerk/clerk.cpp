  ...
  while (true) {
    ...
    if (!ok || reply.err() == ErrWrongLeader) {
      SYLAR_LOG_INFO(g_logger)
          << " 【Clerk::PutAppend】原以为的leader：{" << server << "}请求失败，向新leader{" << server + 1
          << "}重试  ，操作：{" << op << "}";
      if (!ok) {
        SYLAR_LOG_INFO(g_logger) << " 【Clerk::PutAppend】重试原因 ，rpc失敗 ，";
      }
      if (reply.err() == ErrWrongLeader) {
        SYLAR_LOG_INFO(g_logger) << " 【Clerk::PutAppend】重試原因：非leader";
      }
