...
int main(int argc, char** argv) {
    ...
    std::cout << "absolute path=" << sylar::EnvMgr::GetInstance()->getAbsolutePath("ap") << std::endl;
    
    std::cout << "absolute work path=" << sylar::EnvMgr::GetInstance()->getAbsoluteWorkPath("awp") << std::endl;
    std::cout << "config path=" << sylar::EnvMgr::GetInstance()->getConfigPath() << std::endl;
    return 0;
}
