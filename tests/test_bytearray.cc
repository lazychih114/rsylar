...
#define XX(type, len, write_fun, read_fun, base_len) {\
    ...
    SYLAR_LOG_INFO(g_logger) << #write_fun "/" #read_fun \
                    ...
                    << " size=" << ba->getSize(); \
    ba->setPosition(0); \
    SYLAR_ASSERT(ba->writeToFile("/root/rsylar/bin/dats/" #type "_" #len "-" #read_fun ".dat")); \
    sylar::ByteArray::ptr ba2(new sylar::ByteArray(base_len * 2)); \
    SYLAR_ASSERT(ba2->readFromFile("/root/rsylar/bin/dats/" #type "_" #len "-" #read_fun ".dat")); \
    ba2->setPosition(0); \
...
int main(int argc, char** argv) {
    sylar::Thread::SetName("test_bytearray");
    test();
    return 0;
}
